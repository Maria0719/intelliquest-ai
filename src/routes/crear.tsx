...
import { Sparkles, Lightbulb, Wand2, AlertCircle, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { guardarQuizActivo } from "@/servicios/almacenamiento";
...
function Crear() {
  ...
  const puedeGenerar = tema.trim().length > 0 && !cargando;
  ...
  const handleGenerar = async () => {
    ...
    setError(null);
    try {
      const preguntas = await generarQuiz(tema.trim(), cantidad, dificultad);
